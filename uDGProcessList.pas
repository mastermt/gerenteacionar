...
type
  ...
  PUBLIC // methods
    ...
    function Exists(dwProcessID: DWORD; var atIndex: Integer): Boolean;
      OVERLOAD;
    function Exists(const Name: AnsiString): Boolean; OVERLOAD;
    function Exists(const Name: AnsiString; var atIndex: Integer)
      : Boolean; OVERLOAD;
    function ProcessInfoToStr(INDEX: Integer): AnsiString;
    function ProcessInfoToStr2(INDEX: Integer): AnsiString;
    function GetProcessIcon(INDEX: Integer; const bSmallIcon: Boolean = True)
      : TIcon; OVERLOAD;
    ...
    procedure Refresh;
  PUBLIC // properties
    property Process[INDEX: Integer]: TDGProcessRec read GetProcessRec
      write SetProcessRec; DEFAULT;
...
begin
  ...
  for Index := 0 to FList.Count - 1 do
    if Process[Index].ProcessID = dwProcessID then
    begin
      atIndex := Index;
      Exit;
...
begin
  Result := True;
  for Index := 0 to FList.Count - 1 do
    if SameText(Process[Index].Name, Name) then
    begin
      atIndex := Index;
      Exit;
...
begin
  ...
  Result := TIcon.Create;
  SHGetFileInfo(PWideChar(ExeName), 0, FileInfo, SizeOf(FileInfo), Flags);
...
function TDGProcessList.GetProcessStartDateTime(dwProcessID: DWORD): TDateTime;
...
begin
  ...
  if hProcess <> 0 then
  begin
    if GetProcessTimes(hProcess, ftCreationTime, lpExitTime, ftKernelTime,
      ftUserTime) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        bSuccess := GetTokenInformation(hToken, TokenUser, tokUser,
          cbBuf, cbBuf);
      ...
      UserSize := 0;
      DomainSize := 0;
      LookupAccountSid(nil, tokUser.User.Sid, nil, UserSize, nil, DomainSize,
        sidNameUse);
      if (UserSize <> 0) and (DomainSize <> 0) then
      begin
        SetLength(UserName, UserSize);
        SetLength(Domain, DomainSize);
        if LookupAccountSid(nil, tokUser.User.Sid, PWideChar(UserName),
...
function TDGProcessList.ProcessInfoToStr2(INDEX: Integer): AnsiString;
const
  CCRLF = '#';
  // '13#Lista de Processos#' + Description+ '#'+ ExeName + '#'+ (intToStr(ProcID))+'#';
  CPROCESSREC_FMT = '%s' + CCRLF + '%s' + CCRLF + '%d' + CCRLF;
var
  ProcessRec: TDGProcessRec;
begin
  ProcessRec := Process[Index];
  Result := Format(CPROCESSREC_FMT, [ProcessRec.Name,
    // ProcessRec.ExeName,
    ProcessRec.UserName, ProcessRec.ProcessID]);
...
const
  CCRLF = #$D#$A;
  CPROCESSREC_FMT = CCRLF + 'Name = %s' + CCRLF + 'ExeName = %s' + CCRLF +
    'User name = %s' + CCRLF + 'Domain = %s' + CCRLF + 'Started = %s' + CCRLF +
    'Memory usage = %d bytes' + CCRLF + 'Usage = %d' + CCRLF + 'Process ID = %d'
    + CCRLF + 'Default heap ID = %d' + CCRLF + 'Module ID = %d' + CCRLF +
    'Threads = %d' + CCRLF + 'Parent process ID = %d' + CCRLF +
    'Priority base class = %d' + CCRLF;
var
  ProcessRec: TDGProcessRec;
begin
  ProcessRec := Process[Index];
  Result := Format(CPROCESSREC_FMT, [ProcessRec.Name, ProcessRec.ExeName,
    ProcessRec.UserName, ProcessRec.Domain,
    DateTimeToStr(ProcessRec.StartDateTime), ProcessRec.MemoryUsage,
    ProcessRec.Usage, ProcessRec.ProcessID, ProcessRec.DefaultHeapID,
    ProcessRec.ModuleID, ProcessRec.ThreadCount, ProcessRec.ParentProcessID,
...
begin
  ...
  if Process32First(hSnapshot, ProcessEntry32) then
    repeat
      ...
      if GetProcessUserAndDomain(ProcessEntry32.th32ProcessID, UserName, Domain)
      then
      begin
...
{
  How to fill a memo called Memo1 with all processes along with it
  's information procedure TForm1.Button1Click
 (Sender: TObject);
